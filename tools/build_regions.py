...
def write_prov_data():
    """Per-region prov_data.js: entity id -> [source_kind, source, url] so every
    dossier can show where its record came from (data/provenance.csv)."""
    import json
    try:
        import provenance
    except ImportError:
        return
    best = {}
    for row in provenance.read():
        k = (row["region"], row["entity_id"])
        cand = (row["source_kind"], row["source"], row["url"])
        prev = best.get(k)
        if prev is None or cand[2] or not prev[2]:   # rows with a url win; latest otherwise
            best[k] = cand
    for r in REGIONS:
        d = ROOT / r["dir"]
        if not d.exists():
            continue
        m = {eid: v for (reg, eid), v in best.items() if reg == r["dir"]}
        body = ",".join(f'{json.dumps(k)}:{json.dumps(list(v))}' for k, v in sorted(m.items()))
        (d / "prov_data.js").write_text(
            "/* AUTO-GENERATED by tools/build_regions.py from data/provenance.csv — do not edit. */\n"
            "window.PROV={" + body + "};\n", encoding="utf-8")
